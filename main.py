...
def start_command(message):
    ...
    print(f"DEBUG: /start from user id: {user_id}")  # Debug log
...
def gen_command(message):
    ...
    for owner in config.OWNERS:
        try:
            bot.send_message(owner, f"🔑 Keys Generated by {message.from_user.username or message.from_user.first_name} ({user_id}):\n" + "\n".join(generated))
...
def redeem_command(message):
    ...
    if "successfully" in result:
        try:
            bot.send_message(config.OWNERS[0], f"🔔 Key redeemed: {key} by {message.from_user.username or message.from_user.first_name} ({user_id})")
        except Exception as e:
...
def lend_command(message):
    ...
    if target is None:
        ...
        return
    new_points = target[3] + pts
    update_user_points(target_id, new_points)
    bot.reply_to(message, f"✅ {pts} points added to {target_id}. New balance: {new_points}")
    for owner in config.OWNERS:
        try:
            bot.send_message(owner, f"📣 {message.from_user.username} ({user_id}) lent {pts} points to {target_id}.")
        except Exception as e:
            print(f"Error notifying owner: {e}")
...
def notify_command(message):
    ...
    for rec in recipients:
        try:
            bot.send_message(rec, f"📢 Notification from {message.from_user.username} ({user_id}):\n\n{notify_text}", parse_mode="Markdown")
        except Exception as e:
...
def tutorial_command(message):
    text = (
        ...
        "6. Your account info always shows your real-time balance and referral count.\n"
        "Good luck! 😊"
    )
