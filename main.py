...
def gen_command(message):
    ...
    for owner in config.OWNERS:
        bot.send_message(owner, f"🔑 Keys Generated by {message.from_user.username or message.from_user.first_name} ({message.from_user.id}):\n" + "\n".join(generated))
...
def redeem_command(message):
    ...
    if "successfully" in result:
        bot.send_message(config.OWNERS[0], f"🔔 Key redeemed: {key} by {message.from_user.username or message.from_user.first_name} ({message.from_user.id})")
...
def notify_command(message):
    ...
    for rec in recipients:
        bot.send_message(rec, f"📢 Notification from {message.from_user.username} ({message.from_user.id}):\n\n{notify_text}", parse_mode="HTML")
