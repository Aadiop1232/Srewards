...
def send_main_menu(bot, update):
    # Try to extract chat_id and user from both Message and CallbackQuery objects.
    if hasattr(update, "message") and update.message:
        chat_id = update.message.chat.id
        user = update.message.from_user
    elif hasattr(update, "from_user"):
        # For CallbackQuery objects, update usually has from_user.
    ...
    # If the user is an admin, include the Admin Panel button.
    if is_admin(user):
