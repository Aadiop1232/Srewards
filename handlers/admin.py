...
def send_admin_menu(bot, update):
    # Accept update as either a message or a callback query
    ...
    # Show all admin functions to any admin
    markup.add(
    ...
    try:
        ...
        bot.send_message(chat_id, "<b>ðŸ›  Admin Panel</b> ðŸ› ", parse_mode="HTML", reply_markup=markup)
...
def process_admin_ban_unban(bot, message):
    ...
    if row is None:
        ...
        if row[0] == 0:
            ...
            try:
                bot.send_message(user_id, "You have been banned by an administrator.", parse_mode="HTML")
            except Exception as e:
            ...
            try:
                bot.send_message(user_id, "You have been unbanned by an administrator.", parse_mode="HTML")
            except Exception as e:
...
def process_admin_add(bot, message):
    ...
    if len(parts) < 2:
        ...
        try:
            bot.send_message(user_id, "You have been added as an admin.", parse_mode="HTML")
        except Exception as e:
...
def handle_user_management_detail(bot, call, user_id):
    ...
    if not user:
        ...
        return
    # Using order: [0: telegram_id, 1: username, 2: join_date, 3: points, 4: referrals, 5: banned, 6: pending_referrer]
