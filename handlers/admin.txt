...
def is_owner(user_or_id):
    ...
    try:
        ...
        tid = str(user_or_id)
    # Compare directly with config.OWNERS (make sure your owner IDs in config.OWNERS exactly match your Telegram IDs)
    owners = [str(x) for x in config.OWNERS]
    return tid in owners
...
def handle_admin_list(bot, call):
    ...
    if not admins:
        ...
        text = "<b>👥 Admins:</b>\n"
        for admin in admins:
            text += f"• <b>UserID:</b> {admin[0]}, <b>Username:</b> {admin[1]}, <b>Role:</b> {admin[2]}, <b>Banned:</b> {admin[3]}\n"
